...
describe('Handlers', () => {
  ...
  describe('handleAction', () => {
    it('skips hidden triggers', () => {
      ...
      const result = handleAction(new ParserNode(node, defaultContext()), 0);
    ...
    */
    ...
    it('can handle multiple gotos', () => {
      ...
      const result = handleAction(new ParserNode(quest.children().eq(0), defaultContext()), 0);
      expect(result.elem.text()).toEqual('end');
    })
    ...
    it('goes to correct choice', () => {
      ...
      const result = handleAction(new ParserNode(node, defaultContext()), 1);
    ...
    it('goes to correct event', () => {
      ...
      const result = handleAction(new ParserNode(node, defaultContext()), 'test');
      expect(result.elem.text()).toEqual('herp');
    });
    ...
    it('immediately follows triggers on otherwise empty choices', () => {
      ...
      const result = handleAction(new ParserNode(choiceNode, defaultContext()), 0);
