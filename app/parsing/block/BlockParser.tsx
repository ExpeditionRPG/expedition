...
// Does not implement QDLRenderers interface, rather acts as a translation
// between Blocks and inputs that are provided to QDLRenderers.
export class BlockParser {
  constructor () {

  }

  static toRoleplay(blocks: Block[], log: Logger) {
    try {
  ...
  static toQuest(block: Block, log: Logger) {
    block.render = this.renderer.toQuest(this.extractQuestAttributes(block, log));
  }

  static toTrigger(blocks: Block[], log: Logger) {
  ...
  static toCombat(blocks: Block[], log: Logger) {
    try {
      var extracted = this.extractCombatOrRoleplay(blocks[0].lines[0]);
    } catch (e) {
      console.log(e);
  ...
  static finalize(zeroIndentGroups: number[][]): LogMessage[] {
    var toRender: any[] = [];
    var log = new Logger([]);
