...
const skillCheckMatcher = new RegExp(
  `^(${matchEnum(Persona)})?\\s?(${matchEnum(Skill)})?\\s?(${matchEnum(
    Outcome,
  )})?$`,
);
...
export function sanitizeDecision(
  ...
  defaultOutcome: () => any,
  log: Logger,
): { body: TemplateBodyType; attribs: { [k: string]: any } } {
  const sanitized: TemplateBodyType = [];

  const bullets: SkillCheck[] = [];
  for (const section of body) {
    if (!(section as TemplateChild).outcome) {
      // Ignore whitespace
      if (section === '') {
        continue;
      }
  ...
  getPossibleChecks(bullets).forEach(c => {
    const str: string =
      c.persona && c.skill ? `${c.persona} ${c.skill}` : c.skill || '';
    coverage[str] = {};
  });
  ...
  if (numChecks < MIN_CHECKS_FOR_DECISION) {
    log.err(
      `Need at least ${MIN_CHECKS_FOR_DECISION} skill checks, have ${numChecks}`,
      '424',
      line,
    );
  }
