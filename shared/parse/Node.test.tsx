...
describe('Node', () => {
  describe('getNext', () => {
    test('returns next node if enabled', () => {
      const quest = cheerio.load(
        '<quest><roleplay></roleplay><roleplay if="asdf">expected</roleplay><roleplay>wrong</roleplay></quest>',
      )('quest');
    ...
    test('skips disabled elements', () => {
      const quest = cheerio.load(
        '<quest><roleplay if="false">wrong</roleplay><roleplay>expected</roleplay></quest>',
      )('quest');
    ...
    test('displays elements with invalid / undefined ops', () => {
      const quest = cheerio.load(
        '<quest><roleplay>bob</roleplay><roleplay if="erta">expected</roleplay><roleplay>wrong</roleplay></quest>',
      )('quest');
      const pnode = new Node(quest.children().eq(0), defaultContext());
      const next = pnode.getNext();
      if (next === null) {
    ...
    test('returns null if no next element', () => {
      const pnode = new Node(
        cheerio.load('<roleplay></roleplay>')('roleplay'),
        defaultContext(),
      );
    ...
    test('returns next node if choice=0 and no choice', () => {
      const quest = cheerio.load(
        '<quest><roleplay>wrong</roleplay><roleplay>expected</roleplay></quest>',
      )('quest');
      const pnode = new Node(quest.children().eq(0), defaultContext());
    ...
    test('returns node given by choice index', () => {
      const quest = cheerio.load(
        '<quest><roleplay><choice></choice><choice if="false"></choice><choice><roleplay>expected</roleplay><roleplay>wrong</roleplay></choice></roleplay></quest>',
      )('quest');
      const pnode = new Node(quest.children().eq(0), defaultContext());
    ...
    test('returns node given by event name', () => {
      const quest = cheerio.load(
        '<quest><roleplay><event></event><choice></choice><event on="test"><roleplay>expected</roleplay><roleplay>wrong</roleplay></event></roleplay></quest>',
      )('quest');
      const pnode = new Node(quest.children().eq(0), defaultContext());
    ...
    test('safely handles failing to eval conditional choice, logs error', () => {
      const quest = cheerio.load(
        '<quest><roleplay><choice if="notavalue"><roleplay>expected</roleplay></choice><choice><roleplay>bad</roleplay></choice></roleplay></quest>',
      )('quest');
      const pnode = new Node(quest.children().eq(0), defaultContext());
      const next = pnode.getNext(0);
      if (next === null) {
  ...
  describe('getVisibleKeys', () => {
    test('shows events', () => {
      const pnode = new Node(
        cheerio.load(
          '<combat><event on="win"></event><event if="true" on="lose"></event></combat>',
        )('combat'),
        defaultContext(),
      );
    ...
    test('shows choices', () => {
      const pnode = new Node(
        cheerio.load(
          '<roleplay><choice text="a"></choice><choice if="true" text="b"></choice></roleplay>',
        )('roleplay'),
        defaultContext(),
      );
    ...
    test('hides conditionally false events', () => {
      const pnode = new Node(
        cheerio.load(
          '<combat><event if="false" on="win"></event><event on="lose"></event></combat>',
        )('combat'),
        defaultContext(),
      );
    ...
    test('hides conditionally false choices', () => {
      const pnode = new Node(
        cheerio.load(
          '<roleplay><choice text="a" if="false"></choice><choice text="b"></choice></roleplay>',
        )('roleplay'),
        defaultContext(),
      );
  ...
  describe('gotoId', () => {
    test('goes to ID', () => {
      const quest = cheerio.load(
        '<quest><roleplay></roleplay><roleplay>wrong</roleplay><roleplay id="test">expected</roleplay></quest>',
      )('quest');
      const pnode = new Node(quest.children().eq(0), defaultContext());
    ...
    test('returns null if ID does not exist', () => {
      const quest = cheerio.load('<quest><roleplay>wrong</roleplay></quest>')(
        'quest',
      );
      const pnode = new Node(quest.children().eq(0), defaultContext());
    ...
    test('returns null when no <quest> tag', () => {
      const pnode = new Node(
        cheerio
          .load(
            '<roleplay><choice><roleplay id="test">wrong</roleplay></choice></roleplay>',
          )('#test')
          .eq(0),
        defaultContext(),
      );
    ...
    test('safely handles multiple identical ids', () => {
      const quest = cheerio.load(
        '<quest><roleplay></roleplay><roleplay id="test">expected</roleplay><roleplay id="test">expected</roleplay></quest>',
      )('quest');
      const pnode = new Node(quest.children().eq(0), defaultContext());
      const next = pnode.gotoId('test');
      if (next === null) {
  ...
  describe('loopChildren', () => {
    ...
    test('loops only enabled children', () => {
      const pnode = new Node(
        cheerio.load(
          '<roleplay><p>1</p><b>2</b><p if="false">3</p><i>4</i><p if="a">3</p></roleplay>',
        )('roleplay'),
        defaultContext(),
      );
      const agg: any[] = [];
    ...
    test('stops early when a value is returned', () => {
      const pnode = new Node(
        cheerio.load(
          '<roleplay><p>1</p><b>2</b><p if="a">3</p><i>4</i></roleplay>',
        )('roleplay'),
        defaultContext(),
      );
      const result = pnode.loopChildren((tag, c) => {
  ...
  describe('rendering', () => {
    ...
    test('hides children with nothing but variable assignment', () => {
      ...
      expect(
        renderedChildren(
          cheerio.load(
            '<roleplay><p>{{text="TEST"}}</p><p>{{text}}</p></roleplay>',
          )('roleplay'),
          defaultContext(),
        ),
      ).toEqual(['<p>TEST</p>']);
    ...
    test('hides children conditionally', () => {
      ...
      expect(
        renderedChildren(
          cheerio.load(
            '<roleplay><p>{{a=false}}</p><choice if="a" text="Hidden"></choice></roleplay>',
          )('roleplay'),
          defaultContext(),
        ),
      ).toEqual([]);
      ...
      expect(
        renderedChildren(
          cheerio.load(
            '<roleplay><p>{{a=false}}{{b=true}}</p><choice if="a & b" text="Hidden"></choice></roleplay>',
          )('roleplay'),
          defaultContext(),
        ),
      ).toEqual([]);
      ...
      expect(
        renderedChildren(
          cheerio.load(
            '<roleplay><p>{{a=0}}</p><choice if="a" text="Hidden"></choice></roleplay>',
          )('roleplay'),
          defaultContext(),
        ),
      ).toEqual([]);
    ...
    test('shows children conditionally', () => {
      ...
      expect(
        renderedChildren(
          cheerio.load(
            '<roleplay><choice if="a" text="Visible"></choice></roleplay>',
          )('roleplay'),
          defaultContext(),
        ),
      ...
      expect(
        renderedChildren(
          cheerio.load(
            '<roleplay><p>{{a=true}}</p><choice if="a" text="Visible"></choice></roleplay>',
          )('roleplay'),
          defaultContext(),
        ),
      ).toEqual(['<choice if="a" text="Visible"></choice>']);
      ...
      expect(
        renderedChildren(
          cheerio.load(
            '<roleplay><p>{{a=true}}{{b=true}}</p><choice if="a & b" text="Visible"></choice></roleplay>',
          )('roleplay'),
          defaultContext(),
        ),
      ...
      expect(
        renderedChildren(
          cheerio.load(
            '<roleplay><p>{{a=1}}</p><choice if="a" text="Visible"></choice></roleplay>',
          )('roleplay'),
          defaultContext(),
        ),
      ).toEqual(['<choice if="a" text="Visible"></choice>']);
    ...
    test('displays 0 and 1 properly', () => {
      expect(
        renderedChildren(
          cheerio.load(
            '<roleplay><p>{{a=0}}{{b=1}}</p><p>{{a}}{{b}}</p></roleplay>',
          )('roleplay'),
          defaultContext(),
        ),
      ).toEqual(['<p>01</p>']);
    ...
    test('indirects single-valued array results', () => {
      expect(
        renderedChildren(
          cheerio.load('<roleplay><p>{{r=[5]}}</p><p>{{r}}</p></roleplay>')(
            'roleplay',
          ),
          defaultContext(),
        ),
    ...
    test('handles multiple ops on one line', () => {
      expect(
        renderedChildren(
          cheerio.load(
            '<roleplay><p>{{text="TEST"}} {{r=[5]}}</p><p>{{text}}{{r}}</p></roleplay>',
          )('roleplay'),
          defaultContext(),
        ),
      ).toEqual(['<p>TEST5</p>']);
    ...
    test('parses ops in instruction nodes', () => {
      expect(
        renderedChildren(
          cheerio.load(
            '<roleplay><p>{{num = 1}}</p><instruction>Hey {{num}}</instruction></roleplay>',
          )('roleplay'),
          defaultContext(),
        ),
    ...
    test('parses ops in choice attribs', () => {
      expect(
        renderedChildren(
          cheerio.load(
            '<roleplay><p>{{num=1}}</p><choice text="Hey {{num}}"></choice></roleplay>',
          )('roleplay'),
          defaultContext(),
        ),
    ...
    test('parses multi-statement ops in body and respects newlines', () => {
      ...
      expect(
        renderedChildren(
          cheerio.load(
            '<roleplay><p>{{a=5;c=7}}</p><p>{{b=7;a}}</p><p>{{a=5\nb=10}}</p><p>{{a=5\nb=10\nc}}</p></roleplay>',
          )('roleplay'),
          defaultContext(),
        ),
      ).toEqual(['<p>5</p>', '<p>7</p>']);
    ...
    test('increments views.<id>', () => {
      const ctx = defaultContext();
      let result = new Node(
        cheerio.load('<roleplay id="foo"><p>[roll]</p></roleplay>')('roleplay'),
        ctx,
      );
      ...
      result = new Node(
        cheerio.load('<roleplay id="bar"><p>[roll]</p></roleplay>')('roleplay'),
        result.ctx,
      );
    ...
    test('renders deterministically when a seed is given', () => {
      const ctx = defaultContext();
      ctx.seed = 'randomseed';
      const elem = cheerio.load(
        '<roleplay><p>{{a=pickRandom([1,2,3,4,5])}}{{b=round(random(0,100), 4)}}{{c=randomInt(0, 100)}}</p></roleplay>',
      )('roleplay');
      const r1 = new Node(elem, ctx, undefined, ctx.seed);
      const r2 = new Node(elem, ctx, undefined, ctx.seed);

      expect(r1.ctx.scope.a).toEqual(expect.any(Number));
      expect(r1.ctx.scope.b).toEqual(expect.any(Number));
      expect(r1.ctx.scope.c).toEqual(expect.any(Number));
    ...
    test('renders next node via getNext deterministically when a seed is given', () => {
      ...
      const result = new Node(
        cheerio.load(
          '<quest><roleplay></roleplay><roleplay><p>{{a=pickRandom([1,2,3,4,5])}}{{b=round(random(0,100), 4)}}{{c=randomInt(0, 100)}}</p></roleplay></quest>',
        )('quest > :first-child'),
        ctx,
      );
      ...
      expect(n1.ctx.scope.a).toEqual(expect.any(Number));
      expect(n1.ctx.scope.b).toEqual(expect.any(Number));
      expect(n1.ctx.scope.c).toEqual(expect.any(Number));
  ...
  describe('getComparisonKey', () => {
    test('equates separate but identical contexts/node references', () => {
      const e1 = cheerio.load(
        '<quest><roleplay><choice><combat data-line="110"></combat></choice></roleplay></quest>',
      )('combat');
      const p1 = new Node(e1, defaultContext());

      const e2 = cheerio.load(
        '<quest><roleplay><choice><combat data-line="110"></combat></choice></roleplay></quest>',
      )('combat');
    ...
    test('treats different nodes as unequal', () => {
      const dom = cheerio.load(
        '<quest><roleplay><choice><combat id="c1" data-line="110"></combat><combat id="c2" data-line="111"></combat></choice></roleplay></quest>',
      );
      const e1 = dom('#c1');
      const p1 = new Node(e1, defaultContext());
    ...
    test('safely handles lack of quest root', () => {
      const e1 = cheerio.load(
        '<roleplay><choice><combat data-line="110"></combat></choice></roleplay>',
      )('combat');
      const p1 = new Node(e1, defaultContext());
  ...
  describe('getEventParameters', () => {
    test('gets parameters', () => {
      const node = cheerio.load(
        '<combat><event on="win" heal="5" loot="false" xp="false"><roleplay></roleplay></event></combat>',
      )('combat');
    ...
    test('safely handles event with no params', () => {
      const node = cheerio.load(
        '<combat><event on="win"><roleplay></roleplay></event></combat>',
      )('combat');
      const pnode = new Node(node, defaultContext());
      expect(pnode.getEventParameters('win')).toEqual({});
    });
  ...
  describe('handleAction', () => {
    test('renders children deterministically after being called', () => {
      const node = cheerio.load(
        '<roleplay id="start"><choice><roleplay><p>{{random()}}</p></roleplay></choice></roleplay>',
      )('#start');
      const ctx = defaultContext();
    ...
    test('renders using different seed for child card', () => {
      const node = cheerio.load(
        '<roleplay id="start"><p>{{a=random()}}</p><choice><roleplay><p>{{a=random()}}</p></roleplay></choice></roleplay>',
      )('#start');
      const ctx = defaultContext();
    ...
    test('skips hidden triggers', () => {
      const node = cheerio.load(
        '<roleplay><choice><trigger if="false">goto 5</trigger><trigger>end</trigger></choice></roleplay>',
      )('roleplay');
      const pnode = new Node(node, defaultContext());
    ...
    test('uses triggers with invalid / undefined ops', () => {
      const quest = cheerio.load(
        '<quest><roleplay><choice><trigger if="a">goto 5</trigger><trigger>end</trigger><roleplay id="5">expected</roleplay><roleplay>wrong</roleplay></choice></roleplay></quest>',
      )('quest');
      const ctx = defaultContext();
      ctx.scope.a = true;
      const pnode = new Node(quest.children().eq(0), ctx);
    ...
    test('handles invalid triggers', () => {
      const node = cheerio.load(
        '<roleplay><choice><trigger>goto 5</trigger></choice></roleplay>',
      )('roleplay');
      const pnode = new Node(node, defaultContext());
      const result = pnode.handleAction(0);
    ...
    test('handles self-referential triggers', () => {
      const node = cheerio.load(
        '<roleplay><choice><trigger id="5">goto 5</trigger></choice></roleplay>',
      )('roleplay');
      const pnode = new Node(node, defaultContext());
      const result = pnode.handleAction(0);
      expect(result).toEqual(null);
    ...
    test('triggers events', () => {
      ...
      const node = cheerio.load(
        '<roleplay id="rp1"><choice><trigger>end</trigger></choice><choice on="end"><roleplay>expected</roleplay></choice></roleplay>',
      )('#rp1');
      const pnode = new Node(node, defaultContext());
      const result = pnode.handleAction(0);
      if (result === null) {
    ...
    test('can handle multiple gotos', () => {
      const quest = cheerio.load(
        '<quest><roleplay><choice><trigger>goto 1</trigger><trigger id="1">goto 2</trigger><trigger id="2">end</trigger><roleplay>wrong</roleplay></choice></roleplay></quest>',
      )('quest');
      const pnode = new Node(quest.children().eq(0), defaultContext());
      const result = pnode.handleAction(0);
      if (result === null) {
    ...
    test('goes to correct choice', () => {
      const node = cheerio.load(
        '<roleplay><choice></choice><choice><roleplay>herp</roleplay><roleplay>derp</roleplay></choice></roleplay>',
      )('roleplay');
      const pnode = new Node(node, defaultContext());
    ...
    test('goes to next roleplay node', () => {
      const node = cheerio.load(
        '<roleplay id="rp1">rp1</roleplay><roleplay>rp2</roleplay>',
      )('#rp1');
      const pnode = new Node(node, defaultContext());
      const result = pnode.handleAction(1);
      if (result === null) {
    ...
    test('goes to correct event', () => {
      const node = cheerio.load(
        '<roleplay><event></event><event on="test"><roleplay>herp</roleplay><roleplay>derp</roleplay></event></roleplay>',
      )('roleplay');
      const pnode = new Node(node, defaultContext());
    ...
    test('immediately follows triggers on otherwise empty choices', () => {
      ...
      const choiceNode = cheerio.load(
        '<roleplay><choice><trigger>goto jump</trigger></choice></roleplay>',
      )('roleplay');
      const jumpNode = cheerio.load('<roleplay id="jump">Jumped</roleplay>')(
        'roleplay',
      );
    ...
    test('does not immediately follow triggers on non-empty choices', () => {
      const node = cheerio.load(
        '<roleplay><choice><roleplay>Not empty</roleplay><trigger>goto jump</trigger></choice></roleplay><roleplay id="jump">Hello</roleplay>',
      )('roleplay');
      const pnode = new Node(node, defaultContext());
      const result = pnode.handleAction(0);
      if (result === null) {
    ...
    test('handles basic choices deterministically when seed is set', () => {
      const node = cheerio.load(
        '<roleplay><choice><roleplay><p>{{a=round(random(0,100), 4)}}</p></roleplay></choice></roleplay>',
      )('roleplay');
      const pnode = new Node(node, defaultContext());
    ...
    test('handles choices with id gotos deterministically when seed is set', () => {
      ...
      const jumpNode = cheerio.load(
        '<roleplay id="jump"><p>{{a=round(random(0,100), 4)}}</p></roleplay>',
      )('roleplay');
    ...
    test('handles choices with event gotos deterministically when seed is set', () => {
      const node = cheerio.load(
        '<roleplay id="rp1"><choice><trigger>end</trigger></choice><choice on="end"><roleplay><p>{{a=round(random(0,100), 4)}}</p></roleplay></choice></roleplay>',
      )('#rp1');
      const pnode = new Node(node, defaultContext());
      const r1 = pnode.handleAction(0, 'randomseed');
      const r2 = pnode.handleAction(0, 'randomseed');
