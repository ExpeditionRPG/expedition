...
export function subscribeToCreatorsList(mc: any, email: string) {
  ...
  return mc
    .post('/lists/' + Config.get('MAILCHIMP_CREATORS_LIST_ID') + '/members/', {
      email_address: email,
      status: 'subscribed',
    })
    .then((result: any) => {
...
export function getUserQuests(
  ...
  return db.analyticsEvent
    .findAll({
      ...
      attributes: [
        'questID',
        [Sequelize.fn('MAX', Sequelize.col('created')), 'lastPlayed'],
      ],
      group: 'questID',
    })
    .then((results: any[]) => {
      ...
      for (const k of Object.keys(userQuests)) {
        metas.push(
          db.quests
            .findOne({
              where: { partition: Partition.expeditionPublic, id: k },
              limit: 1,
            })
