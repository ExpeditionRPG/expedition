...
function parseSingleSheet(sheetName: string, url: string): Promise<ResultType> {
  return new Promise<ResultType>((resolve, reject) => {
    parseCSV(url, {
      ...
      complete(results: any) {
        ...
        resolve({ cards, translations: translations || undefined });
      },
    });
  });
}
...
function downloadAndProcessSpreadsheet(name: string) {
  ...
  for (const sheetName of Object.keys(source.sheets)) {
    const url =
      ...
      source.key +
      '/pub?output=csv&single=true&gid=' +
...
export function translationsUpdate(
  translations: TranslationsType,
): TranslationsUpdateAction {
...
export function filterAndFormatCards(
  cards: CardType[],
  filters: FiltersState,
): CardType[] {
  if (cards === null) {
  ...
  const cardFilters = ['sheet', 'tier', 'class'].filter(
    (filterName: string) => {
      ...
      const values = value
        ...
        .map((str: string, index: number): string | JSX.Element => {
          if (iconRegex.test(str)) {
            ...
            if (
              filters.theme.current === 'Color' ||
              filters.theme.current === 'BlackAndWhite'
            ) {
              theme = null;
            }
            return icon(
              str.replace(
                iconRegex,
                (match: string) => match.substring(1) + '_small',
              ),
              theme,
              index,
            );
