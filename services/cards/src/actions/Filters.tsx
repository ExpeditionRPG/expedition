...
export function filterChange(
  ...
  return (dispatch: Redux.Dispatch<any>) => {
    ...
    if (name === 'source' && value === 'custom') {
      ...
      cardstype = window.prompt(
        'Please enter a sheet type (Ability, Adventurer, Encounter, Helper, Loot, Persona, Skill)',
      );
      value = window.prompt(
        'Please enter your card sheet publish CSV URL (see "?" in the top right for help)',
        '',
      ) as string;
    }
    ...
    if (name === 'source') {
      dispatch(
        downloadCards(store.getState().filters.source.current, cardstype),
      );
    } else {
      dispatch(
        cardsFilter(store.getState().cards.data, store.getState().filters),
      );
...
export function filtersCalculate(
