  ...
  public render() {
    ...
    for (let i = 0; i < cards.length; i++) {
      ...
      if (renderSettings.showFronts) {
        frontPageList[frontPageList.length - 1].push(
          <CardFront
            key={card.name || i}
            card={card}
      ...
      if (renderSettings.showBacks) {
        if (renderSettings.uniqueBacksOnly && i > 0) {
          if (
            card.naming !== '' ||
            card.class !== cards[i - 1].class ||
            card.tier !== cards[i - 1].tier
          ) {
            backPageList[backPageList.length - 1].push(
              <CardBack
                key={card.name || i}
                card={card}
                translations={this.props.translations}
    ...
    const frontPages = frontPageList.map((cardPage, i) => {
      return (
        <div key={i} className="page fronts">
          {cardPage}
        </div>
      );
    });
    const backPages = backPageList.map((cardPage, i) => {
      return (
        <div key={i + frontPages.length} className="page backs">
          {cardPage}
        </div>
      );
    });
    const pages = [];
    while (
      (renderSettings.showFronts && frontPages.length > 0) ||
    ...
    return (
      <div
        ...
        className={
          ...
          ' ' +
          this.props.filters.export.current +
          (renderSettings.bleed ? ' bleed' : '')
        }
      >
          ...
          <div>
            <div className="printInstructions">
              ...
              <div className="halfWidth">
                <p>
                  <strong>Before you begin:</strong> Many printers (especially
                  consumer printers) don't handle front-back alignment well. You
                  may end up with up to a 1/8" offset between the front and back
                  of cards. If you think this will bug you, you can buy a
                  professionally printed full-color copy at
                  ExpeditionGame.com/store for just $30.
                </p>
                <ol>
                  <li>
                    Download this PDF and take it to your local print shop.
                  </li>
                  <li>
                    Have it printed on heavy white cardstock (ideally 80-pound
                    or heavier).
                  </li>
                  <li>
                    Make sure to print double-sided, flipped on the long edge,
                    at 100% zoom.
                  </li>
                  <li>
                    Cut the cards using a paper cutter. The more precise you
                    are, the easier they'll be to handle later.
                  </li>
                  <li>Secure your cards with a box or rubber band.</li>
                  <li>
                    Accessories: You'll need at least a dozen tracking clips
                    (such as paper clips) and a d20 die.
                  </li>
                  <li>
                    Rules: All of the rules are in the app! Get it for web,
            ...
            <div className="printInstructions">
              ...
              <div className="fullWidth">
                ...
                <p>
                  Expedition is free to use under the Creative Commons BY-NC-SA
                  4.0 license. You can read more about the license at
                ...
                <p>
                  You must also comply with the Laws of Man and Nature. Do not
                  use any form of this game for nefarious purposes like libel,
                  slander, copyright infringement or harassment. If you break
                  the law and get in trouble for it, Expedition is free of all
                  liability.
                </p>
              </div>
            </div>
