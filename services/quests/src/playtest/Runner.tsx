...
function maybeRunMoreWorkers() {
  ...
  const notStarted = Object.keys(state).filter(
    (k: string) => !state[k].started,
  );
...
function renderRunState(id: string, rs: RunState): JSX.Element {
  ...
  return (
    <div>
      <h3>
        <a href={'https://quests.expeditiongame.com/#' + id} target="_blank">
          {rs.title}
        </a>
        &nbsp;by {rs.author}
        &nbsp;({rs.email}) &nbsp;
        {rs.complete
          ? `DONE (${rs.runtimeMillis} ms, ${rs.runtimeLines}/${rs.totalLines} lines)`
          : '...'}
      </h3>
      <ul>{msgs}</ul>
    </div>
...
const Main = (props: Props): JSX.Element => {
  ...
  const overruns = Object.keys(props.state).filter(
    (k: string) => props.state[k].runtimeMillis > OVERRUN_THRESHOLD_MILLIS,
  );
  ...
  if (overruns.length) {
    const completion = overruns
      ...
      .map(
        (k: string) => props.state[k].runtimeLines / props.state[k].totalLines,
      );
    const completionAvg =
      completion.reduce((a: number, b: number) => a + b) / completion.length;
    overrunStats = (
      <span>
        <div>
          Fraction overran:{' '}
          {((100 * overruns.length) / Object.keys(props.state).length).toFixed(
            2,
          )}
          %
        </div>
  ...
  const stats = (
    <div>
      ...
      <div>
        Complete:{' '}
        {
          Object.keys(props.state).filter(
            (k: string) => props.state[k].complete,
          ).length
        }
      </div>
      {overrunStats}
  ...
  for (const id of Object.keys(props.state)) {
    if (props.state[id].messages.length > 0) {
      errorResults.push(
        <span key={id}>{renderRunState(id, props.state[id])}</span>,
      );
    } else {
      cleanResults.push(
        <span key={id}>{renderRunState(id, props.state[id])}</span>,
