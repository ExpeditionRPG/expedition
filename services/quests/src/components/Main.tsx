...
class Main extends React.Component<Props, { hasError: Error | null }> {
  ...
  public render() {
      ...
      return (
        <div className="main loading">
          ...
          <div className="loadPrompt visible">
            <div>
              Try reloading the page - if the error persists, please email
              contact@expeditiongame.com with this page's URL and error message:
            </div>
            <div>
              Error:{' '}
              {(
                this.state.hasError ||
                this.props.editor.fatalError ||
                ''
              ).toString()}
            </div>
          </div>
    ...
    if (this.props.editor.loadingQuest) {
      return (
        <div className="main loading">
          ...
          <div className="loadPrompt delayLoad">
            Not loading? Try disabling your ad blocker AND allowing multiple
            pop-ups (for authentication). If that doesn't work, hit the "Contact
            Us" button in the bottom right - make sure to include the title of
            your quest.
          </div>
        </div>
      );
      ...
      Object.keys(this.props.quest).length === 0
    ) {
    ...
    const header = (
      <div className="header">
        <Button
          className={
            this.props.bottomPanel === 'CONTEXT' ? 'active' : 'inactive'
          }
          onClick={(event: any) => {
            this.props.onPanelToggle('CONTEXT');
          }}
        >
          Context Explorer
        </Button>
        <Button
          ...
          onClick={(event: any) => {
            this.props.onPanelToggle('NOTES');
          }}
        >
          Quest Notes
        </Button>
        <div className="bottomPanel--right">
          <Button
            onClick={(event: any) => {
              this.props.onLineNumbersToggle();
            }}
          >
          ...
          <Button disabled={true}>
            {`Words: ${
              this.props.editor.wordCount > 0
                ? numeral(this.props.editor.wordCount).format('0,0')
                : '-'
            }`}
          </Button>
        </div>
    ...
    if (!this.props.bottomPanel) {
      contents = (
        <div className="contents">
          <QuestIDEContainer />
          <div className="bottomPanel">{header}</div>
        </div>
      );
    } else {
        ...
        <div className="contents">
          ...
          >
            ...
            <div className="bottomPanel">
              {header}
              {this.props.bottomPanel === 'CONTEXT' && (
