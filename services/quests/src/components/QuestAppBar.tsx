...
class QuestAppBar extends React.Component<Props, {}> {
  public state: { menuAnchor: HTMLElement | undefined } = {
    ...
    if (this.props.editor.dirtyTimeout !== null) {
      ...
      return (
        <span className="error saveIndicator">
          <Tooltip title={this.props.quest.saveError}>
            <div>
              <Button disabled={true}>
                <AlertError /> Error: unable to save
              </Button>
            </div>
          </Tooltip>
        </span>
  ...
  private renderPublishButton() {
    let publishButton = (
      <Button
        ...
        onClick={(event: any) =>
          this.props.onMenuSelect('PUBLISH_QUEST', this.props.quest)
        }
      >
        {this.props.quest.published ? 'Update' : 'Publish'}
      </Button>
    );
    const errors = this.props.annotations.filter(
      annotation => annotation.type === 'error',
    );
    const validating = this.props.editor.worker !== null;
    if (validating) {
      publishButton = (
        <span className="validatingButton">
          <Button disabled={true}>
      ...
      publishButton = (
        <span className="errorButton">
          <Button
            onClick={(event: any) =>
              this.props.onViewError(this.props.annotations, this.props.editor)
            }
          >
    ...
    return (
      <AppBar className="quest_app_bar">
        <Toolbar>
          ...
          >
            {this.props.user.lootPoints}{' '}
            <img className="inline_icon" src="images/loot_white_small.svg" />
          </a>
          <span className="email">{this.props.user.email}</span>
          <IconButton onClick={(e: any) => this.handleMenuClick(e)}>
            <ArrowDropDown />
          </IconButton>
          <Menu
          ...
          >
            ...
            <MenuItem
              onClick={() => this.props.onUserDialogRequest(this.props.user)}
            >
              Sign Out
            </MenuItem>
          </Menu>
        ...
        <Toolbar className="toolbar">
          <Button
            onClick={(event: any) =>
              this.props.onMenuSelect('NEW_QUEST', this.props.quest)
            }
          >
            New
          </Button>
          ...
          {Boolean(this.props.quest.published) && (
            <Button
              onClick={(event: any) =>
                this.props.onMenuSelect('UNPUBLISH_QUEST', this.props.quest)
              }
            >
              Unpublish
            </Button>
          )}
          <Tooltip title="View the source file in Google Drive">
            <span>
              <Button
                disabled={this.props.quest.id === null}
                onClick={(event: any) =>
                  this.props.onMenuSelect('DRIVE_VIEW', this.props.quest)
                }
              >
                View in Drive
              </Button>
          ...
          {this.props.quest.published && (
            <Tooltip title="Opens the published version of your quest in the web app">
              <span>
                <Button
                  id="appview"
                  onClick={(event: any) =>
                    this.props.onMenuSelect('APP_VIEW', this.props.quest)
                  }
                >
                  View in App
                </Button>
          ...
          <Button
            onClick={(event: any) =>
              this.props.onMenuSelect('HELP', this.props.quest)
            }
          >
            Help
          </Button>
