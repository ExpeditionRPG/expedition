...
import { clearMultiplayerActions, remoteify } from '../multiplayer/Remoteify';
...
describe('Multiplayer actions', () => {
  ...
  describe('multiplayerNewSession', () => {
    test('creates and connects to a new session', done => {
        ...
        switch (uri) {
          ...
          case MULTIPLAYER_SETTINGS.connectURI:
            return Promise.resolve(
              mockResponse(200, null, { session: 'testsession' }),
            );
          default:
      ...
      Action(multiplayerNewSession, { multiplayer })
        .execute({ id: 'asdf' }, fakeClient, handlers)
        .then(actions => {
  ...
  describe('multiplayerConnect', () => {
    test('connects to a session', done => {
      ...
      const handler = () =>
        Promise.resolve(mockResponse(200, null, { session: 'testsession' }));
      Action(multiplayerConnect, { multiplayer })
        .execute({ id: 'asdf' }, '1234', fakeClient, handler)
        .then(actions => {
          expect(fakeClient.connect).toHaveBeenCalledWith(
            'testsession',
            '1234',
    ...
    test('catches and indicates web errors', done => {
      const handler = () => Promise.reject('nah');
      Action(multiplayerConnect, { multiplayer })
        .execute({ id: 'asdf' }, '1234', null, handler)
        .then(actions => {
          expect(actions[0]).toEqual(
            expect.objectContaining({ type: 'SNACKBAR_OPEN' }),
          );
  ...
  describe('loadMultiplayer', () => {
    test('fetches past sessions by user id', done => {
      ...
      const handler = () =>
        Promise.resolve(mockResponse(200, null, { history: testHistory }));
      Action(loadMultiplayer, { multiplayer })
        .execute({ id: 'asdf' }, handler)
        .then(actions => {
          expect(actions[0]).toEqual(
            expect.objectContaining({ history: testHistory }),
          );
          done();
        })
        .catch(done);
  ...
  describe('setMultiplayerStatus', () => {
    const doTest = () => {
      ...
      const actions = Action(setMultiplayerStatus, { multiplayer }).execute(
        s,
        c,
      );
      return { c, s, actions };
    ...
    test('dispatches status', () => {
      ...
      expect(actions[0]).toEqual(
        expect.objectContaining({ status: expect.objectContaining(s) }),
      );
    });
  });
  ...
  describe('sendStatus', () => {
    const node = new ParserNode(
      cheerio.load('<roleplay data-line="123"></roleplay>')('roleplay'),
      defaultContext(),
    );
    const cid = 2;
    ...
    test('overrides defaults when partial status passed', () => {
      ...
      expect(c.sendEvent).toHaveBeenCalledWith(
        expect.objectContaining({ waitingOn: 'something' }),
        cid,
      );
    });
    test('dispatches status', () => {
      ...
      expect(actions).toContainEqual(
        expect.objectContaining({ type: 'MULTIPLAYER_CLIENT_STATUS' }),
      );
    });
    ...
    test('does nothing when empty client', () => {
      const { c, actions } = doTest(null, multiplayer.instance, undefined, {
        multiplayer: { ...multiplayer, client: null, instance: null },
      });
      expect(c.sendEvent).not.toHaveBeenCalled();
    });
    test('does nothing when empty instance', () => {
      const { c, actions } = doTest(multiplayer.client, null, undefined, {
        multiplayer: { ...multiplayer, client: null, instance: null },
      });
      expect(c.sendEvent).not.toHaveBeenCalled();
    });
        ...
        multiplayer: {
          ...
          clientStatus: {
            'abc|def': {
              ...
              connected: true,
              waitingOn: 'something',
            }, // Disconnected clients are not counted
          },
        },
      });
      expect(c.sendEvent).toHaveBeenCalledWith(
  ...
  describe('handleEvent', () => {
      ...
      store
        ...
        .then(result => {
          expect(store.getActions()).toContainEqual(
            expect.objectContaining({ client: 'abc', instance: 'mmm' }),
          );
          done();
        })
        .catch(done);
    ...
    test('safely handles unknown events', done => {
      ...
      store
        ...
        .then(result => {
          expect(store.getActions()).toContainEqual(
            expect.objectContaining({ type: 'MULTIPLAYER_COMMIT' }),
          );
          done();
        })
        .catch(done);
    ...
    test('resolves and dispatches ACTION events', () => {
      let called = false;
      function testAction(args: { n: number }) {
        called = true;
      }
    ...
    test('rejects ACTIONs and sends status when id is not an increment', done => {
      const c = fakeConnection();
      Action(handleEvent, { multiplayer })
        .execute(
          {
            id: 2,
            event: {
        ...
        .then(result => {
          expect(c.publish).toHaveBeenCalledWith(
            expect.objectContaining({
              event: expect.objectContaining({ type: 'STATUS' }),
            }),
          );
          done();
        })
    ...
    test('handles MULTI_EVENT', done => {
      ...
      Action(handleEvent, { multiplayer })
        ...
        .then(results => {
          ...
          expect(results[results.length - 1].type).toEqual(
            'MULTIPLAYER_MULTI_EVENT',
          );
          done();
        })
        .catch(done);
