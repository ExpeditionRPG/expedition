...
export const toCard = remoteify(function toCard(
  ...
  const questId =
    (state.quest && state.quest.details && state.quest.details.id) || null;
  ...
  if (state.quest && state.quest.node) {
    // In tests, quest.node may not be set up
  ...
  dispatch({
    type: 'NAVIGATE',
    to: { ...a, ts: Date.now(), key: keylist.join('|'), questId },
    dontUpdateUrl: state.settings && state.settings.simulator,
  } as NavigateAction);
  return a;
});
...
export const toPrevious = remoteify(function toPrevious(
  a: ToPreviousArgs,
  dispatch: Redux.Dispatch<any>,
): ToPreviousArgs {
  dispatch({
...
export const toNavCard = remoteify(function toNavCard(
  a: ToNavCardArgs,
  dispatch: Redux.Dispatch<any>,
  getState: () => AppStateWithHistory,
): ToNavCardArgs {
  const name =
    a.name ||
    (getStorageString(NAV_CARD_STORAGE_KEY, 'TUTORIAL_QUESTS') as CardName);
