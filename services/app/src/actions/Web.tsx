...
export const fetchQuestXML = remoteify(function fetchQuestXML(
  a: FetchQuestXMLArgs,
...
export function loadQuestXML(a: {
  details: Quest;
  questNode: Cheerio;
  ctx: TemplateContext;
}) {
  return (dispatch: Redux.Dispatch<any>) => {
    ...
    logEvent('quest', 'quest_start', {
      ...a.details,
      action: a.details.title,
      label: a.details.id,
    });
...
export function submitUserFeedback(a: {
  quest: QuestState;
  ...
  return (dispatch: Redux.Dispatch<any>, getState: () => AppState) => {
    if (
      a.rating &&
      a.rating < 3 &&
      (!a.text || a.text.length < MIN_FEEDBACK_LENGTH)
    ) {
      return alert(
        'Sounds like the quest needs work! Please provide feedback of at least ' +
          MIN_FEEDBACK_LENGTH +
          ' characters to help the author improve.',
      );
    } else if (
      a.rating &&
      a.text.length > 0 &&
      a.text.length < MIN_FEEDBACK_LENGTH
    ) {
      return alert(
        'Reviews must be at least ' +
          MIN_FEEDBACK_LENGTH +
          ' characters to provide value to authors.',
      );
    }
  ...
  return (dispatch: Redux.Dispatch<any>) => {
    return fetch(AUTH_SETTINGS.URL_BASE + '/quest/feedback/' + type, {
      ...
      .then((response: string) => {
        logEvent('feedback', 'user_feedback_' + type, {
          label: data.questid,
          value: data.rating,
        });
      ...
      .catch((error: Error) => {
        ...
        dispatch(
          openSnackbar(Error('Error submitting review: ' + error.toString())),
        );
      });
  };
}
