...
export function extractDecision(node: ParserNode): DecisionState {
  return (
    (node && node.ctx && node.ctx.templates && node.ctx.templates.decision) ||
    EMPTY_DECISION_STATE
  );
}
...
export const initDecision = remoteify(function initDecision(
  a: InitDecisionArgs,
  ...
  const leveledChecks = parseDecisionChecks(
    Math.min(aliveAdventurers, maxRolls),
    seedrandom.alea(a.node.ctx.seed),
    a.node,
  );
...
export function computeSuccesses(
  ...
  const successThreshold =
    SUCCESS_THRESHOLD_MAP[selected.difficulty || 'Medium'];
...
export function computeOutcome(
  ...
  const failures = rolls.reduce(
    (acc, r) => (r < retryThreshold ? acc + 1 : acc),
    0,
  );
  ...
  if (
    successes >= selected.requiredSuccesses ||
    (rolls.length >= aliveAdventurers && !hasInterrupted)
  ) {
    outcome = Outcome.success;
  } else if (failures > 0) {
    outcome = Outcome.failure;
  } else if (
    rolls.length >= aliveAdventurers ||
    (maxRolls && rolls.length >= maxRolls)
  ) {
...
function generateRequiredSuccesses(
  maxRequiredSuccesses: number,
  rng: () => number,
): number {
  return Math.max(
    MIN_REQUIRED_SUCCESSES,
    Math.min(MAX_REQUIRED_SUCCESSES, Math.floor(rng() * maxRequiredSuccesses)),
  );
}

export function generateLeveledChecks(
  maxRequiredSuccesses: number,
  rng: () => number,
): LeveledSkillCheck[] {
  ...
  while (results.length < 3) {
    const gen = {
      ...
      difficulty: choose<keyof typeof Difficulty>(
        Object.keys(Difficulty) as any,
        rng,
      ),
    ...
    for (const r of results) {
      if (
        r.persona === gen.persona &&
        r.skill === gen.skill &&
        r.difficulty === gen.difficulty
      ) {
        continue;
      }
    }
...
export const handleDecisionSelect = remoteify(function handleDecisionSelect(
  a: HandleDecisionArgs,
  dispatch: Redux.Dispatch<any>,
  getState: () => AppStateWithHistory,
): HandleDecisionArgs | null {
  if (!a.node) {
...
function pushDecisionRoll(
  ...
  const hasInterrupted =
    node.getTag() === 'combat' ||
    node.getVisibleKeys().filter(k => k === 'interrupted').length > 0;
...
export const handleDecisionRoll = remoteify(function handleDecisionRoll(
  a: HandleDecisionRollArgs,
  dispatch: Redux.Dispatch<any>,
  getState: () => AppStateWithHistory,
): HandleDecisionRollArgs | null {
  if (!a.node) {
    a.node = getState().quest.node;
  }
...
export const toDecisionCard = remoteify(function toDecisionCard(
  a: ToDecisionCardArgs,
  dispatch: Redux.Dispatch<any>,
  getState: () => AppStateWithHistory,
): ToDecisionCardArgs {
  if (!a.node) {
