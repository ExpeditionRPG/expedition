...
export default function resolveDecision(props: Props): JSX.Element {
  ...
  if (outcome !== null && outcome !== Outcome.retry) {
    ...
    const title = (
      ...
      } as Record<keyof typeof Outcome, string>
    )[outcome];
    return (
      <Card title={title} theme={props.theme} inQuest={true}>
        {instruction ? (
          <Callout icon={formatImg('adventurer', props.theme, false)}>
            {instruction}
          </Callout>
        ) : (
          <p>Nothing happens.</p>
        )}
        <Button onClick={() => props.onCombatDecisionEnd(props.node)}>
          Next
        </Button>
      </Card>
    );
  ...
  const prevSuccesses = computeSuccesses(
    decision.rolls.slice(0, decision.rolls.length - 1),
    selected,
  );
      ...
      <span>
        <h2>
          ...
          <img
            ...
            src={'images/' + formatImg('roll', props.theme) + '.svg'}
          ></img>
        </h2>
        <p>
        ...
        <h2>
          ...
          <img
            className="inline_icon"
            src={'images/' + formatImg('skill', props.theme) + '.svg'}
          ></img>
        </h2>
        <p>
          <strong>Add</strong> the sum of levels of their {selected.skill}{' '}
          skills to their roll.
          {selected.persona &&
            ` Also add 1 if the adventurer's persona type is ${selected.persona}.`}
        </p>
        <h2>
          Resolve{' '}
          <img
            className="inline_icon"
            src={'images/' + formatImg('damage', props.theme) + '.svg'}
          ></img>
        </h2>
        <p>
        ...
        <p>
          ...
          <br />
          and rolling too low may cause bad things to happen.
        </p>
      </span>
    );
  }
  ...
  return (
    <Card
      title={[
        capitalizeFirstLetter(selected.persona),
        capitalizeFirstLetter(selected.skill),
        'Check',
      ]
        .filter(s => s)
        .join(' ')}
    ...
    >
      ...
      <span>
        <Button onClick={() => props.onRoll(props.node, 18)}>
          {roll} ≥ 17
        </Button>
        <Button onClick={() => props.onRoll(props.node, 14)}>
          {roll} 13 - 16
        </Button>
        <Button onClick={() => props.onRoll(props.node, 10)}>
          {roll} 9 - 12
        </Button>
        <Button onClick={() => props.onRoll(props.node, 6)}>
          {roll} 5 - 8
        </Button>
