...
export default function defeat(props: Props): JSX.Element {
  const helpfulHints = [
    <p>
      Remember, you can adjust combat difficulty at any time in the settings
      menu (in the top right).
    </p>,
    <p>
      Don't forget! Healing abilities and loot can be used on all adventurers,
      even those at 0 health.
    </p>,
    <p>Tip: battles are not won by healing, but by defeating the enemy.</p>,
    <p>
      Want to deal more damage? Look for combinations in your abilities - two
      adventurers working together can often do more damage than two alone.
    </p>,
  ];
  ...
  const helpText =
    props.mostRecentRolls &&
    helpfulHints[props.mostRecentRolls[0] % helpfulHints.length];
  ...
  if (props.combat) {
    ...
    if (nextNode && nextNode.isEnd()) {
      retryButton = (
        <Button onClick={() => props.onRetry()}>Retry (heal to full)</Button>
      );
    }
  }
