...
export default function playerTier(props: Props): JSX.Element {
  ...
  if (props.contentSets.has(Expansion.future)) {
    shouldRunDecision =
      props.combat.roundCount % 5 === 0 || props.combat.roundCount % 5 === 3;
  }

  let helpText: JSX.Element = <span></span>;
  const damage = props.combat.mostRecentAttack
    ? props.combat.mostRecentAttack.damage
    : -1;
  ...
  if (props.settings.showHelp) {
    helpText = (
      <span>
        {injured && (
          <p>
            If you reach zero health, you are knocked out. After resolving this
            turn, you cannot play further cards until you are healed by another
            adventurer or revived at the end of the encounter.
          </p>
        )}
        {theHorror && injured && (
          <Callout icon="horror_white">
            <strong>{CONTENT_SET_FULL_NAMES.horror}:</strong> Upon being knocked
            out, reset your persona to Base.
          </Callout>
        )}
      </span>
    );
  ...
  return (
    <Card title="Resolve Damage" theme="dark" inQuest={true}>
      ...
      <Picker
        ...
        id="adventurers"
        onDelta={(i: number) =>
          props.onAdventurerDelta(
            props.node,
            props.settings,
            props.localAliveAdventurers,
            i,
          )
        }
      ...
      >
        ...
        props.numAliveAdventurers === props.localAliveAdventurers ? (
          <span>The number of adventurers &gt; 0 health.</span>
        ) : (
          <span>
            Local adventurers &gt; 0 health.
            <br />({props.numAliveAdventurers} across all devices)
          </span>
        )}
      </Picker>
      {helpText}
      <Button
        className={
          props.numAliveAdventurers === 0 || props.tier === 0 ? 'subtle' : ''
        }
        disabled={props.numAliveAdventurers <= 0 || props.tier <= 0}
        onClick={() =>
          shouldRunDecision
            ? props.onDecisionSetup(props.node, props.seed)
            : props.onNext(props.node, CombatPhase.prepare)
        }
      >
        Next
      </Button>
      <Button
        ...
        disabled={props.numAliveAdventurers <= 0 && props.tier > 0}
        onClick={() =>
          props.onVictory(props.node, props.settings, props.maxTier, props.seed)
        }
      >
        Victory (Tier = 0)
      </Button>
      <Button
        ...
        onClick={() =>
          props.onDefeat(props.node, props.settings, props.maxTier, props.seed)
        }
      >
        Defeat (Adventurers = 0)
      </Button>
    </Card>
  );
}
