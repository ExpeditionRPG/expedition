...
describe('Roleplay actions', () => {
  describe('midCombatChoice', () => {
    ...
    test('goes to win screen on **win**', () => {
      ...
      expect(actions.filter((a: any) => a.type === 'QUEST_NODE')[0].node.ctx.templates.combat.phase).toEqual(CombatPhase.victory);
    });
    ...
    test('goes to lose screen on **lose**', () => {
      ...
      expect(actions.filter((a: any) => a.type === 'QUEST_NODE')[0].node.ctx.templates.combat.phase).toEqual(CombatPhase.defeat);
    });
    ...
    test('goes to next round when pnode.getNext() falls outside of combat scope', () => {
      ...
      expect(actions.filter((a: any) => a.type === 'QUEST_NODE')[0].node.ctx.templates.combat.phase).toEqual('RESOLVE_ABILITIES');
    });
    ...
    test('handles gotos that point to outside of combat and zeros audio', () => {
      ...
      const node = actions.filter((a: any) => a.type === 'QUEST_NODE')[0].node;
