...
import { handleResolvePhase, initCombat } from '../combat/Actions';
...
describe('Roleplay actions', () => {
  describe('midCombatChoice', () => {
    ...
    const newMidCombatNode = () => {
      ...
      baseNode = Action(initCombat as any).execute({
        node: baseNode,
    ...
    test('goes to win screen on **win**', () => {
      const actions = Action(midCombatChoice).execute({
        settings: TEST_SETTINGS,
        node: newMidCombatNode(),
        index: 0,
        maxTier: 0,
        seed: '',
      });
    ...
    test('goes to next round when pnode.getNext() falls outside of combat scope', () => {
      const rp2 = Action(midCombatChoice).execute({
        settings: TEST_SETTINGS,
        node: newMidCombatNode(),
        index: 3,
        maxTier: 0,
