...
describe('Roleplay actions', () => {
  describe('midCombatChoice', () => {
    ...
    test('goes to win screen on **win**', () => {
      ...
      expect(actions.filter((a) => a.type === 'NAVIGATE')[0].to.phase).toEqual(CombatPhase.victory);
    });
    ...
    test('goes to lose screen on **lose**', () => {
      ...
      expect(actions.filter((a) => a.type === 'NAVIGATE')[0].to.phase).toEqual(CombatPhase.defeat);
    });
    ...
    test('ends quest on **end** and zeros audio', () => {
      ...
      expect(actions.filter((a) => a.type === 'NAVIGATE')[0].to.name).toEqual('QUEST_END');
      expect(actions.filter((a) => a.type === 'AUDIO_SET').length).toEqual(1);
    });
    ...
    test('handles gotos that point to outside of combat and zeros audio', () => {
      ...
      expect(actions.filter((a) => a.type === 'NAVIGATE')[0].to.phase).toEqual(RoleplayPhase.default);
      expect(actions.filter((a) => a.type === 'QUEST_NODE')[0].node.elem.text()).toEqual('Outside Roleplay');
      expect(actions.filter((a) => a.type === 'AUDIO_SET').length).toEqual(1);
    });
