...
export function generateIconElements(
  content: string,
  theme: CardThemeType,
): JSX.Element {
  content = (content || '')
    .replace(
      new RegExp(REGEX.ICON.source, 'g'),
      (match: string, group: string): string => {
        ...
        // based on the theme.
        const suffix =
          theme === 'dark' && icon.indexOf('_white') === -1 ? '_white' : '';
        return `<img class="inline_icon" src="images/${icon}${suffix}_small.svg" />`;
      },
    )
    .replace(
      new RegExp(REGEX.ART.source, 'g'),
      (match: string, group: string): string => {
