...
describe('QuestPreview', () => {
  ...
  function setupCustom(
    ...
    const props: Props = {
      ...
      lastLogin: new Date(),
      quest: new Quest({
        ...TUTORIAL_QUESTS.filter(el => el.title === questTitle)[0],
        ...questOverrides,
      }),
  ...
  test('renders selected quest details', () => {
    const quest = TUTORIAL_QUESTS.filter(
      el => el.title === 'Learning to Adventure',
    )[0];
  ...
  test('allows save for offline play', () => {
    const quest = new Quest({
      ...TUTORIAL_QUESTS[0],
      publishedurl: 'http://somenonlocalurl',
    });
  ...
  test('indicates when quest is new', () => {
    const { props, wrapper } = setup({
      quest: new Quest({ ...TUTORIAL_QUESTS[0], created: new Date() }),
    });
  ...
  test('indicates when quest is official', () => {
    const { props, wrapper } = setup({
      quest: new Quest({ ...TUTORIAL_QUESTS[0], official: true }),
    });
  ...
  test('disallows saving local quests for offline play', () => {
    const quest = new Quest({
      ...TUTORIAL_QUESTS[0],
      publishedurl: 'quests/localquest.xml',
    });
    const { props, wrapper } = setup({ quest });
  ...
  test('asks user for confirmation when deleting a saved quest instance', () => {
    ...
    expect(props.onDeleteConfirm).toHaveBeenCalledWith(
      props.quest,
      props.savedInstances[3].ts,
    );
  });

  test('indicates that horror and future expansions are required', () => {
    const quest = TUTORIAL_QUESTS.filter(
      el => el.title === 'Learning 3: The Future',
    )[0];
