    ...
    <Card title="Game Setup">
      ...
      <Button id="selectLocal" onClick={() => props.onLocalSelect()}>
        <div className="questButtonWithIcon">
          ...
          <div className="summary">Play on one device.</div>
        </div>
      </Button>
      <Button
        id="selectOnlineMultiplayer"
        disabled={!props.isLatestAppVersion}
        onClick={() => props.onMultiplayerSelect(props.user)}
      >
        <div className="questButtonWithIcon">
          <div className="title">Online Multiplayer</div>
          {props.isLatestAppVersion && (
            <div className="summary">
              {!props.user || !props.user.loggedIn ? 'Login and sync' : 'Sync'}{' '}
              your app with friends on another device.
            </div>
          )}
          {!props.isLatestAppVersion && (
            <div className="summary">
              Disabled: Please update your app to the latest version or use the
              web app at app.expeditiongame.com.
            </div>
          )}
        </div>
      </Button>
