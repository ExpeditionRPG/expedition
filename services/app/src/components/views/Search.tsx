...
function renderDetails(quest: Quest, order?: string): JSX.Element | null {
  ...
  return (
    <div className={`details ${subfield}`}>
      {subfield === 'ratingavg' && ratingCount >= 1 && (
        <StarRating readOnly={true} value={+ratingAvg} quantity={ratingCount} />
      )}
      {subfield === 'created' &&
        'Published ' + Moment(quest.created).format('MMM YYYY')}
    </div>
  );
}
  ...
  return (
    ...
    >
      ...
      <Button
        ...
        onClick={() => props.toCard('SEARCH_DISCLAIMER')}
        id="signin"
      >
        Sign In
      </Button>
...
export function renderLoading(onReturn: () => any): JSX.Element {
  return (
    <Card title="Loading" className="search_card" onReturn={onReturn}>
    ...
    >
      <div className="searchDescription">
        ...
        <span>
          ...
          <p>
            If you still see no results, file feedback from the top corner menu.
          </p>
        </span>
        <Button
          className="mediumbutton"
          onClick={() => props.toCard('SEARCH_SETTINGS')}
          id="filter"
        >
          Modify Search
        </Button>
      ...
      <Card
        ...
        header={
          <div className="searchHeader">
            <Button className="searchResultInfo" disabled={true}>
              {this.props.results.length} quests for {this.props.players}
              <img className="inline_icon" src="images/adventurer_small.svg" />
              {(this.props.contentSets.has(Expansion.horror) ||
                this.props.contentSets.has(Expansion.future)) && (
                <span> with </span>
              )}
              {this.props.contentSets.has(Expansion.horror) && (
                <img
              ...
              {this.props.contentSets.has(Expansion.future) && (
                <img
                  className="inline_icon"
                  id="searching_future"
                  src="images/future_small.svg"
                />
              )}
              {this.props.contentSets.has(Expansion.scarredlands) && (
                <img
                  className="inline_icon"
                  id="searching_scarredlands"
                  src="images/titanspawn_small.svg"
                />
              )}
            </Button>
            <Button
