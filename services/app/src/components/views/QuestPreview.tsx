...
function renderRequirementsRow(quest: Quest): JSX.Element | null {
  ...
  if (quest.expansionhorror) {
    requires.push(
      <div key={Expansion.horror}>
        <img className="inline_icon" src="images/horror_small.svg" />
        {CONTENT_SET_FULL_NAMES.horror}
      </div>,
    );
  }
  if (quest.expansionfuture) {
    requires.push(
      <div key={Expansion.future}>
        <img className="inline_icon" src="images/future_small.svg" />
        {CONTENT_SET_FULL_NAMES.future}
      </div>,
    );
  }
  if (quest.expansionwyrmsgiants) {
    requires.push(
      <div key={Expansion.wyrmsgiants}>
        <img className="inline_icon" src="images/beast_small.svg" />
        {CONTENT_SET_FULL_NAMES.wyrmsgiants}
      </div>,
    );
  }
  if (quest.expansionscarredlands) {
    requires.push(
      <div key={Expansion.scarredlands}>
        <img className="inline_icon" src="images/titanspawn_small.svg" />
        {CONTENT_SET_FULL_NAMES.scarredlands}
      </div>,
    );
  }
  if (quest.requirespenpaper) {
    requires.push(
      <div key="penpaper">
        <img className="inline_icon" src="images/book_small.svg" />
        Pen and Paper
      </div>,
    );
  }
  ...
  const buttons = saves.map((s, i) => {
    return (
      <div key={i} className="savedQuest">
        <Button
          onClick={e => props.onPlaySaved(s.details.id, s.ts)}
          id={`playsave${i}`}
        >
          {Moment(s.ts).fromNow()}
          &nbsp; (
          {s.pathLen === undefined
            ? 'unknown position'
            : `${s.pathLen.toString()} ${pluralize('choice', s.pathLen || 0)}`}
          {s.savedBytes ? `, ${Math.round(s.savedBytes / 1024)} KB` : ''})
        </Button>
        <IconButton
          onClick={e => props.onDeleteConfirm(quest, s.ts)}
          id={`deletesave${i}`}
        >
          <CloseIcon />
        </IconButton>
...
const QuestPreview = (props: Props): JSX.Element => {
  ...
  if (offlineTS !== null && !props.isDirectLinked) {
    actions.push(
      <Button
        key="play"
        className="bigbutton"
        onClick={e => props.onPlaySaved(quest.id, offlineTS || 0)}
        id="play"
      >
        Play
      </Button>,
    ...
    actions.push(
      <Button
        key="play"
        className="bigbutton"
        onClick={e => props.onPlay(quest, props.isDirectLinked)}
        id="play"
      >
        Play
      </Button>,
  ...
  if (props.savedInstances.length > 0 && lastSaved !== null) {
    actions.push(
      <Button
        key="continue"
        onClick={e => props.onPlaySaved(quest.id, lastSaved)}
        id="playlastsave"
      >
  ...
  if (!quest.publishedurl.startsWith('quests/')) {
    let offlineButton: JSX.Element | null = (
      <Button
        key="offlinesave"
        onClick={e => props.onSave(quest)}
        id="offlinesave"
      >
    ...
    if (offlineTS !== null) {
      offlineButton = (
        <Button
          key="offlinedelete"
          onClick={e => props.onDeleteOffline(quest.id, offlineTS || 0)}
          id="offlinedelete"
        >
          Clear offline state
        </Button>
  ...
  const isNew =
    (props.lastLogin < quest.created ||
      Moment().diff(quest.created, 'days') <= 7) &&
    !props.lastPlayed;
  ...
  return (
    <Card title="Quest Preview">
      <div className="searchDetails">
        ...
        {quest.ratingcount && quest.ratingcount >= 1 ? (
          <StarRating
            readOnly={true}
            value={+ratingAvg}
            quantity={quest.ratingcount}
          />
        ) : (
          ''
        )}
        <div className="indicators">
          {isNew && (
            <div className="inline_icon">
              <img className="inline_icon" src="images/seedling_small.svg" />{' '}
              Published Recently
            </div>
          )}
          {offlineTS && (
            <div className="inline_icon">
              <img className="inline_icon" src="images/offline_small.svg" />{' '}
              Available Offline
            </div>
          )}
          {props.lastPlayed && (
            <div className="inline_icon">
              <img className="inline_icon" src="images/checkmark_small.svg" />{' '}
              Last completed {Moment(props.lastPlayed).fromNow()}
            </div>
          )}
          {quest.official && (
            <div className="inline_icon">
              <img
                ...
                src="images/logo_outline_small.svg"
              />{' '}
              Official Quest
            </div>
          )}
          {quest.partition === Partition.expeditionPrivate && (
            <div className="inline_icon">
              <img className="inline_icon" src="images/private_small.svg" />{' '}
              Private Quest
            </div>
          )}
          {quest.awarded && (
            <div className="inline_icon">
              <img className="inline_icon" src="images/trophy_small.svg" />{' '}
              {quest.awarded}
            </div>
          )}
        </div>
        ...
        <table className="detailsTable">
          <tbody>
            ...
            <tr>
              ...
              <td>{quest.contentrating}</td>
            </tr>
            {quest.mintimeminutes !== undefined &&
              quest.maxtimeminutes !== undefined && (
                <tr>
                  <th>Play time</th>
                  <td>
                    {formatPlayPeriod(
                      quest.mintimeminutes,
                      quest.maxtimeminutes,
                    )}
                  </td>
                </tr>
              )}
            ...
            <tr>
              <th>Last updated</th>
              <td>{Moment(quest.published).format('MMMM D, YYYY h:mm a')}</td>
            </tr>
          </tbody>
        </table>
      </div>
