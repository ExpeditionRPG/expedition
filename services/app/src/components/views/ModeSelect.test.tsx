...
describe('ModeSelect', () => {
  ...
  function setup(overrides?: Partial<Props>) {
    const props: Props = {
      ...
      onLocalSelect: jest.fn(),
      onMultiplayerSelect: jest.fn(),
  ...
  test('disables multiplayer when different app version', () => {
    ...
    expect(
      elem.find('ExpeditionButton#selectOnlineMultiplayer').prop('disabled'),
    ).toEqual(true);
  });
  ...
  test('shows count across all devices', () => {
    ...
    expect(elem.find('PlayerCount#playerCount').text()).toContain(
  ...
  test('hides count across all devices when no multiplayer', () => {
    ...
    expect(elem.find('PlayerCount#playerCount').text()).not.toContain(
      'across all devices',
    );
  });
});
