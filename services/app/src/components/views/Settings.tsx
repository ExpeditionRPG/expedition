...
const difficultyText: { [v: string]: any } = [
  {
    title: 'Story',
    text: "You're here for the story. Enemies go easy on you.",
  },
  {
    title: 'Normal',
    text: 'Expedition as it was meant to be played. Adventurers start here!',
  },
  {
    title: 'Hard',
  ...
  {
    title: 'Impossible',
    text: 'You will almost surely die, so make your death a glorious one!',
  },
];
...
function stringifyContentSet(cs: Expansion[]): string {
  return [Expansion.base, ...cs]
    .map((name: Expansion) => CONTENT_SET_FULL_NAMES[name])
    .join(' + ');
}
...
const Settings = (props: Props): JSX.Element => {
  ...
  const timerIdx = props.settings.timerSeconds
    ? timerValues.indexOf(props.settings.timerSeconds)
    : 0;
  ...
  const expansions = props.multiplayer.session ? (
    <p className="expansionLabel">
      Locally: <strong>{localExpansions}</strong>
      <br />
      All Devices: <strong>{globalExpansions}</strong>
    </p>
  ) : (
    <p className="expansionLabel">
      Currently playing: <strong>{globalExpansions}</strong>
    </p>
  );
  ...
  return (
    <Card title="Settings">
      <Button
        ...
        onClick={() => props.onExpansionSelect()}
      >
        Choose game / expansion
      </Button>
      {expansions}

      <PlayerCount
        id="playerCount"
        localPlayers={props.settings.numLocalPlayers}
        allPlayers={allPlayers}
        onChange={(i: number) => props.onPlayerChange(i)}
      />

      <Checkbox
        id="multitouch"
        label="Multitouch"
        value={props.settings.multitouch}
        onChange={props.onMultitouchChange}
      >
        {props.settings.multitouch
      ...
      <Picker
        label="Difficulty"
        value={difficultyText[difficultyIdx].title}
        onDelta={(i: number) =>
          props.onDifficultyDelta(props.settings.difficulty, i)
        }
      >
      ...
      <Picker
        label="Timer"
        value={timerText[timerIdx].title}
        onDelta={(i: number) => props.onTimerSecondsDelta(timerIdx, i)}
      >
        <div>
          {timerText[timerIdx].text}
          {props.settings.numLocalPlayers === 1 ? (
            <span>
              <br />
              <strong>Solo play:</strong> Timers are doubled.
            </span>
          ) : (
            ''
          )}
      ...
      <Checkbox
        id="sound"
        label="Sound"
        value={props.settings.audioEnabled}
      ...
      <Checkbox
        id="help"
        label="Show Help"
        value={props.settings.showHelp}
        onChange={props.onShowHelpChange}
      >
        {props.settings.showHelp
          ? 'Setup and combat hints are shown.'
          : 'Setup and combat hints are hidden.'}
      </Checkbox>

      <Checkbox
        id="vibration"
        label="Vibration"
        value={props.settings.vibration}
      ...
      <Checkbox
        id="autoroll"
        label="Auto-Roll"
        value={props.settings.autoRoll}
        onChange={props.onAutoRollChange}
      >
        {props.settings.autoRoll
          ? 'Automatically roll for the party when resolving combat.'
          : 'Do not show pre-generated rolls in combat.'}
      </Checkbox>

      <Picker
        label="Font Size"
        value={fontSizeValues[fontSizeIdx]}
        onDelta={(i: number) => props.onFontSizeDelta(fontSizeIdx, i)}
      >
      ...
      <Checkbox
        id="experimental"
        label="Experimental"
        value={props.settings.experimental}
        onChange={props.onExperimentalChange}
      >
        {props.settings.experimental
          ? 'Experimental features are currently enabled.'
          : 'Experimental features are currently disabled.'}
      </Checkbox>

      <div className="version">Expedition App v{VERSION}</div>
      <div className="privacy">
        <a href="#" onClick={() => openWindow(URLS.PRIVACY_POLICY)}>
          Privacy Policy
        </a>
      </div>
      <div className="privacy">
        <a href="#" onClick={() => openWindow(URLS.CODE)}>
          Source Code (GitHub)
        </a>
      </div>
    </Card>
