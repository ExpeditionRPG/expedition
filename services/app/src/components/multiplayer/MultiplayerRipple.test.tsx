...
describe('MultiplayerRipple', () => {
  ...
  test('Creates a ripple on inbound multiplayer event', () => {
    ...
    jest.spyOn(e.instance(), 'start').mockImplementation(() => undefined);
    e.find('MultiplayerRipple')
      .instance()
      .handle('a', { event: 'touchstart', positions: [[0, 0]], id: TEST_ID });
    expect(e.instance().start).toHaveBeenCalledTimes(1);
  });
  test('Colors ripple based on originating peer', () => {
    const { root } = setup();
    root
      .find('MultiplayerRipple')
      .instance()
      .handle('a', { event: 'touchstart', positions: [[0, 0]], id: TEST_ID });
    root.update();
    const r1 = root
      .find('TouchRipple')
      .prop('classes')
      .child.match(/ripplep(\d)/)[1];
    root
      .find('MultiplayerRipple')
      .instance()
      .handle('b', { event: 'touchstart', positions: [[0, 0]], id: TEST_ID });
    root.update();
    const r2 = root
      .find('TouchRipple')
      .prop('classes')
  ...
  test('Handles touchstart with empty position data', () => {
    ...
    root
      .find('MultiplayerRipple')
      .instance()
      .handle('a', { event: 'touchstart', positions: [], id: TEST_ID });
    expect(e.instance().start).toHaveBeenCalledTimes(1);
  });
  test('Ends one ripple event before starting another', () => {
    const { root } = setup();
    const e = root.find('MultiplayerRipple');
    jest.spyOn(e.instance(), 'end');
    root
      .find('MultiplayerRipple')
      .instance()
      .handle('a', { event: 'touchstart', positions: [], id: TEST_ID });
