...
export const mapStateToProps = (state: AppState): StateProps => {
  return {
    ...
    selectedSave:
      state.quest && state.quest.savedTS
        ? { details: state.quest.details, ts: state.quest.savedTS }
        : null,
...
function validateFeedback(text: string): Error | null {
  if (!text) {
    return new Error(
      'Please enter a description so that we can help resolve the issue.',
    );
  }
  if (text.length < MIN_FEEDBACK_LENGTH) {
    return new Error(
      'Issue description must be at least ' +
        MIN_FEEDBACK_LENGTH +
        ' characters to provide value.',
    );
  }
  return null;
}
