  ...
  public render(): JSX.Element {
    return (
      ...
      >
        ...
        <DialogContent className="dialog">{this.content}</DialogContent>
        <DialogActions>
...
> {
  ...
  constructor(props: ExitMultiplayerDialogProps) {
    ...
    this.content = (
      <p>You will be disconnected and returned to the home screen.</p>
    );
  }
...
export class MultiplayerStatusDialog extends React.Component<
...
export class MultiplayerPeersDialog extends React.Component<
...
> {
  ...
  public render(): JSX.Element {
    const peers = Object.keys(this.props.multiplayer.clientStatus).map(
      (k, i: number) => {
        const c = this.props.multiplayer.clientStatus[k];
        return (
          <div key={i}>
            <div>
              <strong>{c.name || 'Player ' + i}</strong>
            </div>
            <div>
              {c.numLocalPlayers} {pluralize('Player', c.numLocalPlayers)}
            </div>
            <br />
          </div>
        );
    ...
    return (
      ...
      >
        ...
        <DialogContent className="dialog">{peers}</DialogContent>
        <DialogActions>
...
export class ExpansionSelectDialog extends React.Component<
...
> {
  public shouldComponentUpdate(nextProps: ExpansionSelectDialogProps) {
    return (
      ...
      nextProps.settings.contentSets.scarredlands !==
        this.props.settings.contentSets.scarredlands ||
      nextProps.settings.contentSets.wyrmsgiants !==
        this.props.settings.contentSets.wyrmsgiants
    );
  }
      ...
      >
        ...
        <DialogContent className="dialog">
          <Button
            ...
            onClick={() =>
              this.props.onExpansionSelect({ horror: false, future: false })
            }
          >
            Base Game
          </Button>
          ...
          <Button
            ...
            className="primary large"
            onClick={() =>
              this.props.onExpansionSelect({ horror: true, future: false })
            }
          >
            Base + Horror
          </Button>
          ...
          <Button
            ...
            className="primary large"
            onClick={() =>
              this.props.onExpansionSelect({ horror: true, future: true })
            }
          >
            Base + Horror + Future
          </Button>
          <br />
          <Checkbox
            id={Expansion.wyrmsgiants}
            label="Of Wyrms & Giants"
            value={this.props.settings.contentSets.wyrmsgiants || false}
            onChange={v =>
              this.props.onExpansionSelect({ wyrmsgiants: v }, false)
            }
          />
          <Checkbox
            id={Expansion.scarredlands}
            label="Scarred Lands"
            value={this.props.settings.contentSets.scarredlands || false}
            onChange={v =>
              this.props.onExpansionSelect({ scarredlands: v }, false)
            }
          />
          <p style={{ textAlign: 'center', marginTop: '1.5em' }}>
            This will only appear once, but you can change it at any time in
            Settings.
          </p>
          <p style={{ textAlign: 'center', marginTop: '1.5em' }}>
...
> extends React.Component<T, {}> {
  ...
  constructor(props: T) {
    ...
    this.content = (
      <p>Expedition can only be played by a maximum of 6 players.</p>
    );
  }
  ...
  public render(): JSX.Element {
    return (
      ...
      >
        ...
        <DialogContent className="dialog">{this.content}</DialogContent>
        <DialogActions>
          <Button id="cancelButton" onClick={() => this.props.onClose()}>
            Cancel
          </Button>
        </DialogActions>
      </Dialog>
...
export class TextAreaDialog<T extends BaseDialogProps> extends React.Component<
  T,
  {}
> {
  protected title!: string;
  ...
  public shouldComponentUpdate(
    nextProps: BaseDialogProps,
    nextState: { text: string },
  ) {
    return (
      nextProps.open !== this.props.open || nextState.text !== this.state.text
    );
  }
...
export class FeedbackDialog extends TextAreaDialog<FeedbackDialogProps> {
  constructor(props: FeedbackDialogProps) {
    ...
    this.content = (
      <p>
        Thank you for taking the time to give us feedback! If you've encountered
        a bug, please include the steps that you can take to reproduce the
        issue.
      </p>
    );
...
export class ReportErrorDialog extends TextAreaDialog<ReportErrorDialogProps> {
  constructor(props: ReportErrorDialogProps) {
    ...
    this.content = (
      <p>
        Thank you for taking the time to report an error! What were you doing
  ...
  public onSubmit() {
    this.props.onFeedbackSubmit(
      ...
      this.props.settings,
      this.props.user,
      this.state.text + '... Error: ' + this.props.error,
    );
  }
}
...
export class ExitQuestDialog extends TextAreaDialog<ExitDialogProps> {
  constructor(props: ExitDialogProps) {
    ...
    this.content = (
      <p>
        You will lose your place in the quest and return to the home screen.
      </p>
    );
  ...
  constructor(props: FeedbackDialogProps) {
    ...
    this.content = (
      <span>
        ...
        <ul>
          <li>
            Offensive or inappropriate for the age level it claimed to be.
          </li>
...
export class SetPlayerCountDialog extends React.Component<
...
> {
  public render(): JSX.Element {
    ...
    if (quest.minplayers && quest.maxplayers) {
      playersAllowed =
        allPlayers >= quest.minplayers && allPlayers <= quest.maxplayers;
    }
    let contents = (
      <div>
        {!playersAllowed &&
          `Quest requires ${quest.minplayers} - ${quest.maxplayers} players.`}
        <PlayerCount
          id="playerCount"
          localPlayers={this.props.settings.numLocalPlayers}
          allPlayers={allPlayers}
          onChange={(i: number) => this.props.onPlayerChange(i)}
        />
        <Checkbox
          id="multitouch"
          label="Multitouch"
          value={this.props.settings.multitouch}
          onChange={this.props.onMultitouchChange}
        >
          {this.props.settings.multitouch
            ? 'All players must hold their finger on the screen to end combat.'
            : 'A single tap will end combat.'}
        </Checkbox>
      </div>
    );

    let expansionErr = '';
    const contentSets = new Set<Expansion>(
      getContentSets(this.props.settings, this.props.multiplayer),
    );
    ...
    if (quest.expansionhorror && !contentSets.has(Expansion.horror)) {
      ...
      quest.expansionwyrmsgiants &&
      !contentSets.has(Expansion.wyrmsgiants)
    ) {
      expansionErr = CONTENT_SET_FULL_NAMES.wyrmsgiants;
    } else if (
      quest.expansionscarredlands &&
      !contentSets.has(Expansion.scarredlands)
    ) {
    ...
    if (expansionErr) {
      contents = (
        <div className="error">
          The {expansionErr} expansion is required to play this quest. If you
          have it, make sure to enable it in settings. Otherwise, you can pick
          up a copy on
          <a
            href="#"
            onClick={() =>
    ...
    return (
      ...
      >
        <DialogTitle>
          {expansionErr ? 'Expansion Required' : 'How many players?'}
        </DialogTitle>
        <DialogContent className="dialog">{contents}</DialogContent>
        <DialogActions>
          <Button onClick={() => this.props.onClose()}>Cancel</Button>
          {!expansionErr && (
            <Button
              id="play"
              disabled={!playersAllowed}
              className="primary"
              onClick={() => this.props.playQuest(this.props.quest)}
            >
              Play
            </Button>
          )}
