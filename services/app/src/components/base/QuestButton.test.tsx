...
describe('QuestButton', () => {
  ...
  test('displays no book icon when a quest does not require pen and paper', () => {
    const html = setup({
      quest: new Quest({ ...TUTORIAL_QUESTS[0], requirespenpaper: false }),
    }).e.html();
  ...
  test('does not display awarded icon when quest not awarded', () => {
    const html = setup({
      quest: { ...TUTORIAL_QUESTS[0], awarded: null },
    }).e.html();
  ...
  test('displays awarded icon if quest has received award', () => {
    const html = setup({
      quest: {
        ...TUTORIAL_QUESTS[0],
        awarded: 'The Bob Medal For Questing Mediocrity',
      },
    }).e.html();
  ...
  test('does not display official icon if quest is not official', () => {
    const html = setup({
      quest: { ...TUTORIAL_QUESTS[0], official: false },
    }).e.html();
  ...
  test('displays official icon if quest is official', () => {
    const html = setup({
      quest: { ...TUTORIAL_QUESTS[0], official: true },
    }).e.html();
  ...
  test('shows private icon when quest is private', () => {
    const html = setup({
      quest: { ...TUTORIAL_QUESTS[0], partition: Partition.expeditionPrivate },
    }).e.html();
